...
datapath = os.path.join(nsml.DATASET_PATH, 'train')
...
def train(fps, args):
  with tf.compat.v1.name_scope('loader'):
    x = loader.decode_extract_and_batch(
        ...
        prefetch_size=args.train_batch_size * 4)[:, :, 0]
  ...
  with tf.compat.v1.train.MonitoredTrainingSession(
    ...
    print('-' * 80)
    print('Training has started. Please use \'tensorboard --logdir={}\' to monitor.'.format(train_dir))
    while True:
      # Train discriminator
...
"""
def incept(args):
  incept_dir = os.path.join(train_dir, 'incept')
...
#      help='Training directory')
  ...
  if mode == 'train':
    fps = glob.glob(os.path.join(data_dir, '*'))
    if len(fps) == 0:
